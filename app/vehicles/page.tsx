...
export default function VehiclesPage(){
 ...
 const activeStock=vehicles.filter(v=>v.status!=="Sold"),available=vehicles.filter(v=>v.status==="Available").length,transit=vehicles.filter(v=>v.status==="In Transit").length,value=activeStock.reduce((s,v)=>s+Number(v.totalCost||0),0);
 ...
 return <>
  ...
  <div className="inventory-summary"><div className="mini-stat"><Car/><span><small>Total vehicles</small><b>{vehicles.length}</b></span></div><div className="mini-stat"><MapPin/><span><small>Available</small><b>{available}</b></span></div><div className="mini-stat"><Car/><span><small>In transit</small><b>{transit}</b></span></div><div className="mini-stat"><Wallet/><span><small>Active stock value</small><b>{money(value)}</b></span></div></div>
