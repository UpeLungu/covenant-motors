...
export default function VehiclesPage(){
 ...
 <section className="panel inventory-panel">
  ...
  <div className="desktop-stock-table table-wrap"><table><thead><tr><th>Stock ID</th><th>Vehicle</th><th>Registration</th><th>Status</th><th>Cost Price</th><th>Estimated Selling Price</th><th>Actions</th></tr></thead><tbody>{filtered.map(v=><tr key={v.id}><td><b>{v.stockId}</b></td><td><div className="vehicle-cell"><div className="vehicle-thumb"><Car size={19}/></div><span><b>{v.make} {v.model}</b><small>{v.year}</small></span></div></td><td><b>{v.registrationNumber||"Not registered"}</b></td><td><span className={`badge ${badge(v.status)}`}>{v.status}</span></td><td><b>{formatCurrency(cost(v),"ZMW")}</b></td><td><b>{value(v)?formatCurrency(value(v),"ZMW"):"Not valued"}</b></td><td><div className="row-actions"><Link className="button secondary compact-button" href={`/vehicles/${v.id}`}><Eye size={14}/>View</Link><Link className="button secondary compact-button" href={`/vehicles/${v.id}/edit`}><PenLine size={14}/>Edit</Link><button className="button secondary compact-button" type="button" onClick={()=>setEditing(v)}><Wallet size={14}/>Value</button>{v.status!=="Sold"&&<Link className="button compact-button" href={`/sales?vehicle=${v.id}`}><ShoppingCart size={14}/>Sell</Link>}</div></td></tr>)}</tbody></table></div>
