...
export const loadAccounts=()=>load<Account>(ACCOUNTS_KEY,starterAccounts).map(x=>({...x,currency:x.currency||"ZMW",openingBalance:Number(x.originalOpeningBalance??x.openingBalance??0)}));
export const saveAccounts=(x:Account[])=>localStorage.setItem(ACCOUNTS_KEY,JSON.stringify(x));
export const loadFinanceEntries=()=>load<FinanceEntry>(ENTRIES_KEY,[]).map(x=>{const amount=Number(x.originalAmount??x.moneyIn??x.moneyOut??0),incoming=Number(x.moneyIn||0)>0;return {...x,currency:x.currency||"ZMW",originalAmount:amount,moneyIn:incoming?amount:0,moneyOut:incoming?0:amount}});
export const saveFinanceEntries=(x:FinanceEntry[])=>localStorage.setItem(ENTRIES_KEY,JSON.stringify(x));
export const accountBalance=(account:Account,entries:FinanceEntry[])=>Number(account.openingBalance||0)+entries.filter(e=>e.accountId===account.id&&(e.currency||"ZMW")===(account.currency||"ZMW")).reduce((s,e)=>s+Number(e.moneyIn||0)-Number(e.moneyOut||0),0);
