"use client";
export type Account={id:string;name:string;type:"Bank"|"Cash"|"Mobile Money";openingBalance:number;currency?:"ZMW";openingBalanceZmw?:number};
export type FinanceEntry={id:string;date:string;type:"Customer Payment"|"Supplier Payment"|"General Expense"|"Capital"|"Other Income";reference:string;description:string;accountId:string;moneyIn:number;moneyOut:number;currency?:"ZMW";moneyInZmw?:number;moneyOutZmw?:number;relatedId?:string;createdAt:string};
...
export const loadAccounts=()=>load<Account>(ACCOUNTS_KEY,starterAccounts).map(x=>({...x,currency:"ZMW",openingBalance:Number(x.openingBalanceZmw??x.openingBalance??0)}));
...
export const loadFinanceEntries=()=>load<FinanceEntry>(ENTRIES_KEY,[]).map(x=>({...x,currency:"ZMW",moneyIn:Number(x.moneyInZmw??x.moneyIn??0),moneyOut:Number(x.moneyOutZmw??x.moneyOut??0)}));
