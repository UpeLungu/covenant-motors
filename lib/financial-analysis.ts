...
import {CurrencyTotals,toZmw} from "./currency";

export function vehicleCostZmw(vehicle:Vehicle,expenses:VehicleExpense[]):number{
 const originalPurchase=Number(vehicle.originalPurchasePrice??vehicle.purchasePrice??0);
 const purchase=Number(vehicle.purchasePriceZmw??toZmw(originalPurchase,vehicle.purchaseCurrency||"ZMW",vehicle.purchaseExchangeRate));
...
export function saleProfitAnalysis(sale:Sale,vehicle:Vehicle|undefined,expenses:VehicleExpense[]){
 ...
 const originalSale=Number(sale.originalSellingPrice??sale.sellingPrice??0);
 const revenue=Number(sale.sellingPriceZmw??toZmw(originalSale,sale.currency||"ZMW",sale.exchangeRate));
