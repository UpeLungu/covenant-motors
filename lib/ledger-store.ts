...
const demoDocuments:VehicleDocument[]=[{id:"vd1",vehicleId:"1",type:"Purchase Invoice",name:"Purchase invoice",reference:"INV-001",date:"2026-07-10",createdAt:"2026-07-10T09:00:00Z"},{id:"vd2",vehicleId:"1",type:"Registration",name:"Registration record",reference:"BBA 1234",date:"2026-07-13",createdAt:"2026-07-13T09:00:00Z"}];
...
export const loadVehicleExpenses=()=>load<any>(EXPENSE_KEY,demoExpenses).map((x:any)=>({...x,amount:Number(x.originalAmount??x.amount??0),currency:x.currency||"ZMW"} as VehicleExpense));
export const saveVehicleExpenses=(x:VehicleExpense[])=>localStorage.setItem(EXPENSE_KEY,JSON.stringify(x));
export const loadVehicleDocuments=()=>load<VehicleDocument>(DOCUMENT_KEY,demoDocuments);export const saveVehicleDocuments=(x:VehicleDocument[])=>localStorage.setItem(DOCUMENT_KEY,JSON.stringify(x));
